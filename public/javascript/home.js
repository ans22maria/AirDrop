$().ready(function () {
  ...
  // Listener to capture the submission of the ticket reservation form.
  $('#reserve').on('submit', function (event) {
    event.preventDefault();
    let formDataArray = $('#reserve').serializeArray();
    let formData = {};
    $.each(formDataArray, function (index, item) {
      formData[item.name] = item.value;
    });
    console.log(formData);
    formData.flight_id=formData.destinationSelector;
    formData.destinationSelector = $('#destinationSelector').find(':selected').data('destination');
